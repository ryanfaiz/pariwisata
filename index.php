<?php
    ...
    $result = mysqli_query($conn, "SELECT * FROM acara WHERE utama = 'Iya'");
?>
...
<head>
    ...
    <title>Home | Kementerian Pariwisata</title>
...
<body>
    ...
    <section class="introduction">
        <video src="/asset/video/intro.webm" autoplay muted loop></video>
        <div class="absolute-con">
            <h2>Welcome to Indonesia</h2>
            <h4>Kementerian Pariwisata Republik Indonesia</h4>
        </div>
    </section>

    <section class="covid">
        <div class="title">
            <h3>Health Protocols You Must Obey</h3>
        </div>

        <div class="protocol-wrapper">
            <div class="protocol">
                <div class="protocol-con">
                    <img src="/asset/img/health-protocol-1.webp" alt="">
                    <h4>Wear Mask</h4>
                </div>
    
                <div class="protocol-con">
                    <img src="/asset/img/health-protocol-2.webp" alt="">
                    <h4>75% Maximum</h4>
                </div>
    
                <div class="protocol-con">
                    <img src="/asset/img/health-protocol-3.webp" alt="">
                    <h4>Remind Each Other</h4>
                </div>
            </div>
        </div>
    </section>
        ...
        <?php while ( $row = mysqli_fetch_row($result)) : ?>
            <div class="event-wrapper">
                ...
                <div class="event-con">
                    ...
                    <h2><?= $row[3]; ?></h2>
                    <h2><?= $row[2]; ?></h2>
                    <h4 id="countdown<?= $row[0]; ?>"></h4>
                </div>
            </div>
            <script>
                var countDownDate<?= $row[0]; ?> = new Date("<?= $row[3]; ?> 00:00:00").getTime();
                var x<?= $row[0]; ?> = setInterval(function() {
                var now<?= $row[0]; ?> = new Date().getTime();
                var distance<?= $row[0]; ?> = countDownDate<?= $row[0]; ?> - now<?= $row[0]; ?>;

                var days<?= $row[0]; ?> = Math.floor(distance<?= $row[0]; ?> / (1000 * 60 * 60 * 24));
                var hours<?= $row[0]; ?> = Math.floor((distance<?= $row[0]; ?> % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                var minutes<?= $row[0]; ?> = Math.floor((distance<?= $row[0]; ?> % (1000 * 60 * 60)) / (1000 * 60));
                var seconds<?= $row[0]; ?> = Math.floor((distance<?= $row[0]; ?> % (1000 * 60)) / 1000);

                document.getElementById("countdown<?= $row[0]; ?>").innerHTML = days<?= $row[0]; ?> + " hari " + hours<?= $row[0]; ?> + " jam "
                + minutes<?= $row[0]; ?> + " menit " + seconds<?= $row[0]; ?> + " detik";

                if (distance<?= $row[0]; ?> < 0) {
                    clearInterval(x);
                    document.getElementById("countdown<?= $row[0]; ?>").innerHTML = "Event Telah Berakhir";
                }
                }, 1000);
            </script>
        <?php endwhile; ?>
    ...
    <section class="teams">
        ...
        <div class="staff-wrapper">
            <div class="staff-con">
                <img src="/asset/img/pp.webp" alt="">
                <h4>Christiant Danu Adnand N.</h4>
            </div>

            <div class="staff-con">
                <img src="/asset/img/ryan.webp" alt="">
                <h4>Ryan Faiz Sanie</h4>
            </div>

            <div class="staff-con">
                <img src="/asset/img/pp.webp" alt="">
                <h4>Stanis Laus Abies P.</h4>
            </div>
        </div>
    </section>
